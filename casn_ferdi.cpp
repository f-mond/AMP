...
//			checking if the values were reset in case of fail, or if they got changed in case of success
			...
			for(int k=0;k<exp_values.size();k++){
//			checking if the values were reset in case of fail, or if they got changed in case of success
			if(((ran2 && *ent2[i].addr!=ent2[i].new1) || (!ran2 && *ent2[i].addr != exp_values[k] && ran_shared2[j*2+abs(id-1)] !=true)) && incl2[k] == 1) std::cout << "check failed operation 2 thread " << id << " on entry " << i << " should be " << (ran2? ent2[i].new1 : exp_values[k]) << " but is " << (*ent2[i].addr) << " " << ran2 << " " << ran_shared2[j*2+abs(id-1)] << std::endl;
			if(incl[k] == 1) i++;
			}
